...
export default function Carousel({ trending }) {
  ...
  const moveNext = () => {
    if (
      currentIndex <
      Math.floor(maxScrollWidth.current / carousel.current.offsetWidth)
    ) {
      setCurrentIndex((prevState) => prevState + 1);
    }
  };
  ...
  useEffect(() => {
    ...
    if (carousel.current !== null) {
      maxScrollWidth.current = 3813 - carousel.current.offsetWidth;
    }
  }, []);
  ...
  return (
    <div className="carousel my-5 mx-auto h-[150px]">
      <div className="relative overflow-hidden">
        ...
        <div
          ref={carousel}
          className="h-[150px] carousel-container relative flex gap-1 overflow-hidden scroll-auto z-0"
        >
